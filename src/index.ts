...
import { getAssignedPRs, postPRReviews } from './azure/pr';
...
async function main() {
	...
	try {
		...
		// 3. Find PRs assigned to user
		const assignedPrs = await getAssignedPRs(gitApi, config.azureRepositoryId, config.azureProjectName, currentUser.uniqueName);

		if (assignedPrs.length === 0) {
			console.log('No PRs require review from this user. Exiting.');
			return;
		}
		...
		for (const pr of assignedPrs) {
			...
			try {
				...
				await postPRReviews(gitApi, config.azureRepositoryId, config.azureProjectName, pr, suggestedReviews, config);
			} catch (err) {
